//
//  DatabaseMasnager.swift
//  LetsChat
//
//  Created by Chuong Tran on 28.3.2021.
//
...
extension DatabaseManager {
    ...
    /// Inserts new user to database
       public func insertUser(with user: LetsChatUser, completion: @escaping (Bool) -> Void) {
    ...
    public func createNewConversation(with otherUserEmail: String, name: String, firstMessage: Message, completion: @escaping(Bool) -> Void){
        ...
        ref.observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            if var conversations = userNode["conversations"] as? [[String: Any]] {
                ...
                /*
                 Conversation array dose not exists
                 create it
                 */
...
//    Fetches and return all conversations for user with passed in email
    ...
    //Get all messages for giving conversation
    ...
    //Sends a conversation with targer conversation and message
        ...
        database.child("\(conversation)/messages").observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            case .photo(let mediaItem):
                if let targetUrlString = mediaItem.url?.absoluteString{
                    ...
                    customMessage = "Sent photo"
                }
                break
            case .video(let mediaItem):
                if let targetUrlString = mediaItem.url?.absoluteString {
                    message = targetUrlString
                    customMessage = "Sent video"
                }
                break
            case .location(_):
                break
            ...
            strongSelf.database.child("\(conversation)/messages").setValue(currentMessages) { error, _ in
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                    ...
                    let updatedValue: [String:Any] = [
                        "date": dateString,
                        "is_read": false,
                        "message": customMessage.count == 0 ? message : customMessage
                    ]
...
struct LetsChatUser {
    ...
    var profilePictureFileName: String {
        //testuser-gmail-com_profile_picture.png
